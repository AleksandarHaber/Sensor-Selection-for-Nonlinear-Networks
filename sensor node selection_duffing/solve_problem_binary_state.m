...
% - input parameters: 
%                   - computed_output_sequence    - output sequence simulated
%                   from the estimated initial state - NOTE: in the
%                   original version of the code that is used for the
%                   memory network, this variable was a simulated sequence,
%                   however, since the i-th local state of every system is
%                   multiplied by the block [sensor_variable(i) 0] of the output C matrix, this is equivalent to having only
%                   an ouput sequence.
...
function [solution]=solve_problem_binary_state(computed_output_sequence,output_sequence, no_observed_nodes)


[N,time_horizon]=size(computed_output_sequence);
...
for i=1:time_horizon 
   A2=[A2;kron(diag(computed_output_sequence(:,i)),[-1;1])];
end
